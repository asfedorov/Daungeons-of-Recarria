...
class HelloWorld(cocos.layer.Layer):
    ...
    def __init__(self):
        ...
        for i, j, col in map_field._iter_tiles_from_field():
            ...
            sprite_obj.position = self._pos_offset + (self._tile_size[0]*j), self._pos_offset + (self._tile_size[1]*i)
            sprite_obj.rotation = random.choice((0, 90, 180))
            ...
            if col == map_gen.WALL_TILE:
                ...
                sprite_obj_w.position = (
                    self._pos_offset + (self._tile_size[0]*j),
                    self._pos_offset + (self._tile_size[1]*i)
                )
                sprite_obj_w.rotation = 0
                self.add(sprite_obj_w, 1)
            # else:
